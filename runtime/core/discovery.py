...
class TemplateDiscovery(DiscoveryInterface):
    ...
    async def discover_templates(self, scan_paths: List[Path]) -> List[TemplateInfo]:
        ...
        if not templates:
            logger.info("File-based discovery found no templates, falling back to inheritance-based discovery")
    ...
    def _is_template_class(self, cls: Type) -> bool:
        ...
        try:
            ...
            return (inspect.isclass(cls) and 
                    issubclass(cls, BaseAgentTemplate) and 
                    not inspect.isabstract(cls) and
                    cls is not BaseAgentTemplate)
        except ImportError:
